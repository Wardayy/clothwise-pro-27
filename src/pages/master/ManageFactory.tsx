...
export default function ManageFactory() {
  ...
  const refresh = () => qc.invalidateQueries({ queryKey: ['factories'] });

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !location.trim()) { toast.error('Fill all fields'); return; }
    try {
      if (editId) { await store.updateFactory(editId, { factory_name: name, location }); toast.success('Factory updated'); setEditId(null); }
      else { await store.addFactory({ factory_name: name, location }); toast.success('Factory added'); }
  ...
  const handleDelete = async (id: string) => {
    try { await store.deleteFactory(id); toast.success('Deleted'); refresh(); }
    catch (e: any) { toast.error(e.message); }
  };
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
            <div className="space-y-1 flex-1"><Label>Factory Name</Label><Input value={name} onChange={e => setName(e.target.value)} /></div>
            <div className="space-y-1 flex-1"><Label>Location</Label><Input value={location} onChange={e => setLocation(e.target.value)} /></div>
